...
public class ForgeEntrypoint {
    ...
    private void sendSetupLines(CommandSourceStack source, List<SetupLine> lines) {
        for (var line : lines) {
            ...
            Style style;
            if (line.isCopy()) {
                style = Style.EMPTY.withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, line.copyValue()));
            } else {
                style = Style.EMPTY.withClickEvent(new ClickEvent(
                        line.suggest ? ClickEvent.Action.SUGGEST_COMMAND : ClickEvent.Action.RUN_COMMAND,
                        line.clickCommand));
            }
            if (line.hoverText != null) {
                style = style.withHoverEvent(
                        new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(line.hoverText)));
            }
